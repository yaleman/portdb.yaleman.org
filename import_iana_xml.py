...
def writefile(filetowrite: str | Path, contents: str) -> None:
    """open and write a file"""

    with Path(filetowrite).open("w", encoding="utf8") as file_handle:
        file_handle.write(contents)


def mkservicedir(servicedata: IndividualService) -> str:
    """generate a service dir"""
    return f"data/{servicedata['protocol']}/{servicedata['port']}/"


def ianafilename(iana_service: IndividualService) -> str:
    """generate an iana service filename"""
    return f"{mkservicedir(iana_service)}iana.md"


def buildmd(servicedata: IndividualService) -> str:
    """generate a markdown file"""
    markdown = f"_Name:_ {servicedata['name']}\n\n"
    ...
    for _, service in info.items():
        ...
        if os.path.exists(mkservicedir(service)):
            ...
            if ianafile.exists():
                ...
                if ianafile_contents == md.encode("utf8"):
                    noupdate += 1
