"""Render Zola content from port source data."""
...
def generate_protocol(protocol: str, data_dir: Path, content_dir: Path) -> int:
...
def generate_content(data_dir: Path = Path("data"), content_dir: Path = Path("content")) -> int:
    """Generate all protocol pages and return the total page count."""
    return sum(generate_protocol(protocol, data_dir, content_dir) for protocol in PROTOCOLS)
